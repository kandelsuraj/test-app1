...
(function () {
  ...
  function setup(root) {
    ...
    var piecesLabel = piecesProp ? (/^properties\[(.*)\]$/.exec(piecesProp.name) || [])[1] || null : null;
    ...
    function displayValue(control, field) {
      ...
      if (type === 'radio') {
        var picked = control.querySelector('input:checked');
        return picked ? picked.parentNode.textContent.trim() : '';
      }
    ...
    function update() {
      ...
      if (rawInput) {
        rawInput.value = JSON.stringify({
          values: variables,
          pieces: pieces,
          // Lets the cart change pieces later without the product page.
          piecesLabel: piecesLabel,
          maxPieces: piecesInput ? maxPieces : 1,
    ...
    if (piecesInput) {
      var steppers = Array.prototype.slice.call(root.querySelectorAll('[data-price-calc-step]'));

      var syncSteppers = function () {
        var pieces = Math.floor(Number(piecesInput.value)) || 1;
        steppers.forEach(function (button) {
          var down = Number(button.getAttribute('data-price-calc-step')) < 0;
          button.disabled = down ? pieces <= 1 : pieces >= maxPieces;
        });
      };

      steppers.forEach(function (button) {
        button.addEventListener('click', function () {
          var pieces = (Math.floor(Number(piecesInput.value)) || 1) + Number(button.getAttribute('data-price-calc-step'));
          piecesInput.value = String(Math.min(Math.max(pieces, 1), maxPieces));
          piecesInput.dispatchEvent(new Event('change', { bubbles: true }));
        });
      });

      ['input', 'change'].forEach(function (type) {
        piecesInput.addEventListener(type, function () {
          syncSteppers();
          update();
        });
      });
    ...
    // Coming back from the cart, browsers bring the page back with the last
    // order's values still filled in: from the back/forward cache, or by
    // refilling the form on reload. Start every visit from the defaults.
    var form = root.querySelector('[data-price-calc-form]');
    function resetForm() {
      if (form) form.reset();
      if (piecesInput) piecesInput.dispatchEvent(new Event('change'));
      else update();
    }
    ...
    var navigation = window.performance && performance.getEntriesByType
      ...
      : null;
    if (navigation && navigation.type === 'back_forward') resetForm();
